...
public class Wheel1 {
    ...
    public Wheel1(float posX, float posY)
    {
       sound=Gdx.audio.newSound(Gdx.files.internal("Swords_Collide-Sound_Explorer-2015600826.wav"));
        position=new Vector2(posX,posY);
        velocity=new Vector2((float)0.12*Gdx.graphics.getWidth(),0);
        acceleration=new Vector2(0,(float)-0.277*Gdx.graphics.getHeight());
        sprite=new Sprite(new Texture(Gdx.files.internal("wheel.png")));
        sprite.setPosition(posX,posY);
        boundary=new Circle(posX+103,posY+103,93);
    ...
    public void spriteCollisionDetection(Sprite framesprite)
    {
        ...
        {
            sound.play();
            velocity.y=(float)0.4589*Gdx.graphics.getHeight();
            position.y=27;
            elapsedTimeY=0;
        }
