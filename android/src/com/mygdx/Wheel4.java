...
public class Wheel4 {
    ...
    public Wheel4(float posX, float posY) {
        sound = Gdx.audio.newSound(Gdx.files.internal("Swords_Collide-Sound_Explorer-2015600826.wav"));
        position = new Vector2(posX, posY);
        velocity = new Vector2((float) 0.12 * Gdx.graphics.getWidth()*(float)0.8, 0);
        acceleration = new Vector2(0, (float) -0.22 * Gdx.graphics.getHeight());
        sprite = new Sprite(new Texture(Gdx.files.internal("wheel4.PNG")));
        sprite.setPosition(posX, posY);
        boundary=new Circle(posX+196,posY+196,187);
    }
