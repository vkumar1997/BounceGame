...
public class PlayScreen implements Screen, InputProcessor {
    ...
    int game_time=22,frames=0, game_level, NO_OF_FIRES=9;

    public PlayScreen(BounceGame game, int game_level)
    {
        this.game=game;
        ...
        if(game_level==4) {
            w4.add(new Wheel4((float) 0.23 * Gdx.graphics.getWidth(),(float) 0.486 * Gdx.graphics.getHeight()));
            w4.get(0).velocity.y=(float) 0.4589 * Gdx.graphics.getHeight()/(float)1.1;
            w4.get(0).acceleration.y=0;
            game_time = 40;
            NO_OF_FIRES = 20;
        }
        ...
        if(game_level==3) {
            w1.add(new Wheel1((float) 0.46 * Gdx.graphics.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
            w1.add(new Wheel1((float) 0.8 * Gdx.graphics.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
    ...
    public void render(float delta) {
        ...
        if(!start)
        {
            foreback.draw(batch);
            if (game_level==1) {
                bitmapFont.draw(batch, "Level 1", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Hello World!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "Turn your device to control his motions.\nTouch to shoot.", foreback.getX() + 50, foreback.getY() + 50);
            }
            else if (game_level==4) {
                bitmapFont.draw(batch, "Level 4", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Constant Velocity!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "The wheel does not accelerates or de-accelerates", foreback.getX() + 50, foreback.getY() + 50);
            }
            else
            {
                bitmapFont.draw(batch, "Level 3", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Blink On And Off!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "The wheels become invisible regularly\nBut they do exist.", foreback.getX() + 50, foreback.getY() + 50);
            }
        }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (collide)
        {
            ...
            {
                Score.total_score=0;
                game.setScreen(new PlayScreen(game, game_level));
            }
        ...
        if (w1.isEmpty() && w2.isEmpty() && w3.isEmpty() && w4.isEmpty())
        {
            if(screenX>=foreback.getX()+50&&screenY>=foreback.getY()+20 && screenX<=(foreback.getX()+200)&&screenY<=(foreback.getY()+70))
            {
                if(game_level==1 || game_level==4)
                    game.setScreen(new PlayScreen1(game,game_level+1));
                if(game_level==3)
                    game.setScreen(new PlayScreen(game,game_level+1));
            }
           else if(screenX>=(2*foreback.getX()+foreback.getWidth())/2+50 && screenY>=(foreback.getY()+20)&& screenX<=(2*foreback.getX()+foreback.getWidth())/2+200&&screenY<=(foreback.getY()+70))
            {
                Score.total_score=0;
    ...
    public void getWheelPosition()
    {
        ...
        if (!collide && start && !w4.isEmpty()) {
            for (Wheel4 wheel : w4) {
                ...
                wheel.elapsedTimeX += Gdx.graphics.getDeltaTime();
                wheel.elapsedTimeY += Gdx.graphics.getDeltaTime();
                if(game_level==4 && wheel.sprite.getY()+wheel.sprite.getHeight()==Gdx.graphics.getHeight()-27)
                {
                    wheel.velocity.y=-(float) 0.4589 * Gdx.graphics.getHeight()/(float)1.1;
                }
                wheel.sprite.setPosition(wheel.position.x + wheel.velocity.x * wheel.elapsedTimeX, wheel.position.y + wheel.velocity.y * wheel.elapsedTimeY + (float) 1 / 2 * wheel.acceleration.y * wheel.elapsedTimeY * wheel.elapsedTimeY);
    ...
    public void drawWheelPosition()
    {
       if(!w1.isEmpty()) {
           for (Wheel1 wheel : w1) {
               if((game_time%2!=0 || collide) && game_level==3)
        ...
        if(!w2.isEmpty()) {
            for (Wheel2 wheel : w2) {
                if((game_time%3!=0 || collide) && game_level==3)
                    wheel.sprite.draw(batch);
                else if(game_level!=3)
                    wheel.sprite.draw(batch);
                wheel.sprite.rotate(2.7f);
        ...
        if(!w3.isEmpty()) {
            for (Wheel3 wheel : w3) {
                if((game_time%4!=0 || collide) && game_level==3)
                    wheel.sprite.draw(batch);
                else if(game_level!=3)
                    wheel.sprite.draw(batch);
                wheel.sprite.rotate(2.7f);
        ...
        if(!w1.isEmpty()) {
            for (ListIterator<Wheel1> iterator=w1.listIterator(); iterator.hasNext();) {
                ...
                if(Intersector.overlaps(wheel.boundary,fireboundary))
                {
                    ...
                    w2.get(w2.size()-1).velocity.y=Math.abs(wheel.velocity.y);
                    if (game_level==4) {
                        w2.get(w2.size() - 1).acceleration.y = 0;
                        w2.get(w2.size() - 2).acceleration.y = 0;
                        w2.get(w2.size()-1).velocity.x=(float) 0.12 * Gdx.graphics.getWidth()*(float)2.8;
                        w2.get(w2.size()-2).velocity.x=-(float) 0.12 * Gdx.graphics.getWidth()*(float)2.8;
                    }
                    iterator.remove();
                    return true;
                }
        ...
        if(!w2.isEmpty()) {
            for (ListIterator<Wheel2> iterator=w2.listIterator(); iterator.hasNext();) {
                ...
                if(Intersector.overlaps(wheel.boundary,fireboundary))
                {
                    ...
                    w3.get(w3.size()-1).velocity.y=Math.abs(wheel.velocity.y);
                    if(game_level==4) {
                        w3.get(w3.size() - 1).acceleration.y = 0;
                        w3.get(w3.size() - 2).acceleration.y = 0;
                        w3.get(w3.size()-1).velocity.x=(float) 0.12 * Gdx.graphics.getWidth()*(float)2.3;
                        w3.get(w3.size()-2).velocity.x=-(float) 0.12 * Gdx.graphics.getWidth()*(float)2.3;
                    }
                    iterator.remove();
                    return true;
                }
        ...
        if(!w4.isEmpty()) {
            for (ListIterator<Wheel4> iterator=w4.listIterator(); iterator.hasNext();) {
                ...
                if(Intersector.overlaps(wheel.boundary,fireboundary))
                {
                    ...
                    iterator.remove();
                    if (game_level==4) {
                        w1.get(w1.size() - 1).acceleration.y = 0;
                        w1.get(w1.size() - 2).acceleration.y = 0;
                    }
                    return true;
                }
            }
