...
public class PlayScreen6 implements Screen, InputProcessor {
    BounceGame game;
    ...
    Sprite sprite, firesprite, frameSprite, foreback, bordersprite;
    int no_of_fires=0;
    ArrayList<Float> f=new ArrayList();
    int game_time=65,frames=0, game_level, NO_OF_FIRES=30, misfires;

    public PlayScreen6(BounceGame game, int game_level)
    {
        this.game=game;
        ...
        if(game_level==10) {
            w2.add(new Wheel2((float) 0.8 * Gdx.graphics.getWidth(),(float) 0.486 * Gdx.graphics.getHeight()));
            w2.add(new Wheel2((float) 0.8 * Gdx.graphics.getWidth(),(float) 0.486 * Gdx.graphics.getHeight()));
            w2.get(0).velocity.x=-w2.get(0).velocity.x;
            w2.get(0).velocity.y=-(float)0.4589*Gdx.graphics.getHeight()/(float)1.1;
            w2.get(0).acceleration.y=-w2.get(0).acceleration.y;
            w2.add(new Wheel2((float) 0.8 * Gdx.graphics.getWidth(),(float) 0.486 * Gdx.graphics.getHeight()));
            w2.get(2).velocity.x=-w2.get(2).velocity.x;
        }
        ...
        bordersprite=new Sprite(new Texture(Gdx.files.internal("border.PNG")));
        bordersprite.setSize(20,Gdx.graphics.getHeight()-40);
    ...
    public void render(float delta) {
        frames=frames+1;
        if (frames>=60 && !collide && start && (!w2.isEmpty() || !w3.isEmpty() ))
        {
            frames=0;
        ...
        bitmapFont.draw(batch,"Time~ "+game_time,Gdx.graphics.getWidth()/2-(int)(0.07*Gdx.graphics.getWidth()),Gdx.graphics.getHeight()-50);
        ...
        bordersprite.setPosition(frameSprite.getX() + frameSprite.getWidth() / 2, frameSprite.getY() + 20);
        ...
        if (no_of_fires==NO_OF_FIRES && (!w2.isEmpty() ||  !w3.isEmpty() ) && f.isEmpty() && !collide)
        {
            collide=true;
            frames=0;
        }
        ...
        if(!start)
        {
            foreback.draw(batch);
            if (game_level==10) {
                bitmapFont.draw(batch, "Level 10", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Mario Mirrored!!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "Consider the first frame the mirror image of second frame\nEvery two misfire generates a new wheel", foreback.getX() + 50, foreback.getY() + 50);
            }
        }
        ...
        if (w2.isEmpty() && w3.isEmpty())
        {
            ...
            foreback.draw(batch);
            bitmapFont.draw(batch, "Game Complete!!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
            bitmapFont.draw(batch,"Score~ "+score.getScore(),foreback.getX()+40,(2*foreback.getY()+foreback.getHeight())/2+50);
            if (frames>0 && frames<20) {
                Score.total_score = score.getTotalScore() + score.getScore();
                frames=20;
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if(no_of_fires!=NO_OF_FIRES && start && !collide &&  (!w2.isEmpty() || !w3.isEmpty() )) {
        ...
        if (collide)
        {
            ...
            {
                Score.total_score=0;
                game.setScreen(new PlayScreen6(game, game_level));
            }
        ...
        if (w2.isEmpty() && w3.isEmpty())
        {
            if(screenX>=(2*foreback.getX()+foreback.getWidth())/2-50 && screenY>=(foreback.getY()+20)&& screenX<=(2*foreback.getX()+foreback.getWidth())/2+100&&screenY<=(foreback.getY()+70))
            {
                Score.total_score=0;
                game.setScreen(new MainScreen(game));
            }
    ...
    public void getWheelPosition()
    {
        ...
        if (!collide && start && !w2.isEmpty()) {
            for (Wheel2 wheel : w2) {
                wheel.spriteCollisionDetection(bordersprite,frameSprite,3);
    ...
    {
        if (touchDown && !collide && start && (!w2.isEmpty() || !w3.isEmpty())) {
        ...
        else if (!collide &&  ( !w2.isEmpty() || !w3.isEmpty() )) {
            if ((deviceAngle <= 10 && deviceAngle >= -10) || x >= bordersprite.getX()-sprite.getWidth()-33 || x <= 33 || !start) {
            ...
            if (deviceAngle >10 && x  <= bordersprite.getX()-sprite.getWidth()-33  && start) {
                x = x + (int) (0.35 * (deviceAngle));
    ...
    public void drawFirePosition()
    {
        ...
        if(!f.isEmpty() && !collide && start && elapsedTime>=animation3.getFrameDuration()/(float)(1.5)) {
            ...
            for(ListIterator<Float> iterator=f.listIterator(); iterator.hasNext();) {
                ...
                iterator.set(fy + 7f);
                iterator.previous();
                fireboundary=new Rectangle(frameSprite.getWidth()-(fx+22),fy+60,18,50);
                boolean collision=fireCollision();
                if (w2.isEmpty()&& w3.isEmpty()  && collision)
                    frames=0;
                iterator.next();
