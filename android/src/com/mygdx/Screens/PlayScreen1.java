...
public class PlayScreen1 implements Screen, InputProcessor{
    ...
    int game_time=50,frames=0,no_of_fires=0, game_level,NO_OF_FIRES=28;
    ArrayList<Float> f=new ArrayList();


    public PlayScreen1 (BounceGame game,int game_level)
    {
        this.game=game;
        ...
        w1.add(new Wheel1((float) 0.3906 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
        w1.add(new Wheel1((float) 0.5468 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
        w1.add(new Wheel1((float) 0.703125 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
        w1.add(new Wheel1((float) 0.9* frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
        w1.get(1).velocity.x=-w1.get(1).velocity.x;
        w1.get(3).velocity.x=-w1.get(3).velocity.x;
        game_time=70;
        NO_OF_FIRES=45;}

        if (game_level==2)
        {w4.add(new Wheel4((float) 0.23 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
        w1.add(new Wheel1((float) 0.46 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));}
    ...
    public void render (float delta)
    {
        ...
        if(!start)
        {
            if(game_level==2) {
                foreback.draw(batch);
                bitmapFont.draw(batch, "Level 2", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Bigger Dimensions!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
            ...
            {
                foreback.draw(batch);
                bitmapFont.draw(batch, "Level 5", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Quickly and Carefully!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "Turn your device to control his motions.\nTouch to shoot.", foreback.getX() + 50, foreback.getY() + 50);
            }
        }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (collide)
        {
            ...
            {
                Score.total_score=0;
                game.setScreen(new PlayScreen1(game,game_level));
            }
        ...
        if (w1.isEmpty() && w2.isEmpty() && w3.isEmpty() && w4.isEmpty())
        {
            if(screenX>=foreback.getX()+50&&screenY>=foreback.getY()+20 && screenX<=(foreback.getX()+200)&&screenY<=(foreback.getY()+70))
            {
                ...
                if(game_level==5)
                    game.setScreen(new PlayScreen4(game,6));
            }
            else if(screenX>=(2*foreback.getX()+foreback.getWidth())/2+50 && screenY>=(foreback.getY()+20)&& screenX<=(2*foreback.getX()+foreback.getWidth())/2+200&&screenY<=(foreback.getY()+70))
            {
                Score.total_score=0;
