 ...
 */
public class PlayScreen3 implements Screen, InputProcessor{
    BounceGame game;
    SpriteBatch batch;
    BitmapFont bitmapFont,bitmapFont2;
    Sprite frameSprite, sprite, foreback, firesprite;
    ...
    int game_time=80,split_time=0,frames=0,no_of_fires=0, game_level,NO_OF_FIRES=50;
    ArrayList<Float> f=new ArrayList();


    public PlayScreen3 (BounceGame game,int game_level)
    {
        this.game=game;
        ...
        frameSprite=new Sprite(new Texture(Gdx.files.internal("frame2.PNG")));
        frameSprite.setPosition(0, 0);
        frameSprite.setSize((float)1.5 * Gdx.graphics.getWidth(), Gdx.graphics.getHeight());


        if (game_level==8)
        {
            w4.add(new Wheel4((float) 0.23 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
        }
    ...
    public void render (float delta)
    {
        ...
        if(split_time>=6 && w3.size()<4)
        {
            sound.play();
            if(!w4.isEmpty())
            {
                w4.add(new Wheel4(w4.get(0).sprite.getX(),w4.get(0).sprite.getY()));
                w4.get(w4.size()-1).velocity.x=-w4.get(0).velocity.x;
                w4.get(w4.size()-1).velocity.y=Math.abs(w4.get(0).velocity.y);
                w4.get(0).velocity.y=Math.abs(w4.get(0).velocity.y);
            }
            else if(!w1.isEmpty())
            {
                w1.add(new Wheel1(w1.get(0).sprite.getX(),w1.get(0).sprite.getY()));
                w1.get(w1.size()-1).velocity.x=-w1.get(0).velocity.x;
                w1.get(w1.size()-1).velocity.y=Math.abs(w1.get(0).velocity.y);
                w1.get(0).velocity.y=Math.abs(w1.get(0).velocity.y);
            }
            else if(!w2.isEmpty())
            {
                w2.add(new Wheel2(w2.get(0).sprite.getX(),w2.get(0).sprite.getY()));
                w2.get(w2.size()-1).velocity.x=-w2.get(0).velocity.x;
                w2.get(w2.size()-1).velocity.y=Math.abs(w2.get(0).velocity.y);
                w2.get(0).velocity.y=Math.abs(w2.get(0).velocity.y);
            }
            else if(!w3.isEmpty())
            {
                w3.add(new Wheel3(w3.get(0).sprite.getX(),w3.get(0).sprite.getY()));
                w3.get(w3.size()-1).velocity.x=-w3.get(0).velocity.x;
                w3.get(w3.size()-1).velocity.y=Math.abs(w3.get(0).velocity.y);
                w3.get(0).velocity.y=Math.abs(w3.get(0).velocity.y);
            }
            split_time=0;
        }
        batch.begin();
        ...
        bitmapFont.draw(batch, "Score~ " + score.getScore()+w3.size(), 50, Gdx.graphics.getHeight() - 50);
        ...
        if(!start)
        {
            if(game_level==8) {
                foreback.draw(batch);
                bitmapFont.draw(batch, "Level 8", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Exponential growth!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "The wheel divides in two of same size. So shoot quickly\n Priority on bigger balls", foreback.getX() + 50, foreback.getY() + 50);
            }
        }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (collide)
        {
            ...
            {
                Score.total_score=0;
                game.setScreen(new PlayScreen3(game,game_level));
            }
        ...
        if (w1.isEmpty() && w2.isEmpty() && w3.isEmpty() && w4.isEmpty())
        {
            if(screenX>=foreback.getX()+50&&screenY>=foreback.getY()+20 && screenX<=(foreback.getX()+200)&&screenY<=(foreback.getY()+70))
            {
                game.setScreen(new PlayScreen5(game,game_level+1));
            }
            else if(screenX>=(2*foreback.getX()+foreback.getWidth())/2+50 && screenY>=(foreback.getY()+20)&& screenX<=(2*foreback.getX()+foreback.getWidth())/2+200&&screenY<=(foreback.getY()+70))
            {
                Score.total_score=0;
    ...
    public void drawMarioPosition()
    {
        if (touchDown && !collide && start && (!w1.isEmpty() || !w2.isEmpty() || !w3.isEmpty() || !w4.isEmpty())) {
            ...
            if (deviceAngle < -10 && x <= Gdx.graphics.getWidth() - sprite.getWidth() - 60  &&start) {
                ...
                mario.setPosition(x + 8, 25);
                translate=false;
            } else if (deviceAngle > 10 && x >= 50  && start) {
                x = x - (int) (0.35 * (deviceAngle));
                ...
                mario.setPosition(x + 8, 25);
                translate=false;
            } else if (deviceAngle < -10 && x > Gdx.graphics.getWidth() - sprite.getWidth() - 60 && frameSprite.getX() >= -frameSprite.getWidth()/3  &&start) {
                sprite.setRegion(animation1.getKeyFrame(elapsedTime, true));
                frameSprite.translateX((int) (0.35 * deviceAngle));
                sprite.draw(batch);
                translate=true;
            } else if (deviceAngle > 10 && x < 50
                    && frameSprite.getX() <= 0  && start) {
                sprite.setRegion(animation2.getKeyFrame(elapsedTime, true));
                frameSprite.translateX((int) (0.35 * deviceAngle));
                sprite.draw(batch);
                translate=true;
