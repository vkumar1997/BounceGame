 ...
 */
public class PlayScreen5 implements Screen, InputProcessor{
    BounceGame game;
    SpriteBatch batch;
    ...
    int game_time=70,frames=50,no_of_fires=0, game_level,NO_OF_FIRES=30;
    ArrayList<Float> f=new ArrayList();


    public PlayScreen5 (BounceGame game,int game_level)
    {
        this.game=game;
        ...
        if (game_level==9)
        {
            ...
            w4.get(1).setFrame(2);
            w4.add(new Wheel4((float) 0.8 * frameSprite.getWidth(), (float) 0.486 * Gdx.graphics.getHeight()));
            w4.get(2).setFrame(3);
        }
    ...
    {
        frames=frames+1;
        if (frames>=60 && !collide && start && (!w1.isEmpty() || !w2.isEmpty()  || !w4.isEmpty()))
        {
            frames=0;
        ...
        if (no_of_fires==NO_OF_FIRES && (!w1.isEmpty() || !w2.isEmpty() || !w4.isEmpty()) && f.isEmpty() && !collide)
        {
            collide=true;
            frames=0;
        }
        ...
        if(!start)
        {
            if(game_level==9) {
                foreback.draw(batch);
                bitmapFont.draw(batch, "Level 9", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth(), foreback.getY() + foreback.getHeight() - 40);
                bitmapFont.draw(batch, "Bulls Eye!", (2 * foreback.getX() + foreback.getWidth()) / 2 - (float) 0.1 * Gdx.graphics.getWidth() - 50, foreback.getY() + foreback.getHeight() - 90);
                bitmapFont.draw(batch, "Touch to begin!!", foreback.getX() + 50, foreback.getY() + 130);
                bitmapFont2.draw(batch, "Hit right in the middle of wheel. Use fireballs carefully\nSmallest wheels do not exist", foreback.getX() + 50, foreback.getY() + 50);
            }
        }


        if (w1.isEmpty() && w2.isEmpty()  && w4.isEmpty())
        {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if(no_of_fires!=NO_OF_FIRES && start && !collide &&  (!w1.isEmpty() || !w2.isEmpty() || !w4.isEmpty())) {
        ...
        if (collide)
        {
            ...
            {
                Score.total_score=0;
                game.setScreen(new PlayScreen5(game,game_level));
            }
        ...
        if (w1.isEmpty() && w2.isEmpty() && w4.isEmpty())
        {
            if(screenX>=foreback.getX()+50&&screenY>=foreback.getY()+20 && screenX<=(foreback.getX()+200)&&screenY<=(foreback.getY()+70))
            {
                game.setScreen(new PlayScreen6(game,game_level+1));
            }
            else if(screenX>=(2*foreback.getX()+foreback.getWidth())/2+50 && screenY>=(foreback.getY()+20)&& screenX<=(2*foreback.getX()+foreback.getWidth())/2+200&&screenY<=(foreback.getY()+70))
            {
                Score.total_score=0;
    ...
    {
        if (touchDown && !collide && start && (!w1.isEmpty() || !w2.isEmpty() || !w4.isEmpty())) {
        ...
        else if (!collide &&  (!w1.isEmpty() || !w2.isEmpty() || !w4.isEmpty())) {
    ...
    public void drawFirePosition()
    {
        ...
        if(!f.isEmpty() && !collide && start && elapsedTime>=animation3.getFrameDuration()/(float)(1.5)) {
            ...
            for(ListIterator<Float> iterator=f.listIterator(); iterator.hasNext();) {
                ...
                if (w1.isEmpty() && w2.isEmpty() && w4.isEmpty() && collision)
                    frames=0;
                iterator.next();
        ...
        if(!w1.isEmpty()) {
            for (ListIterator<Wheel1> iterator=w1.listIterator(); iterator.hasNext();) {
                ...
                if(Intersector.overlaps(wheel.boundary, fireboundary))
                {
                    ...
                    w2.get(w2.size()-1).boundary.radius=10;
                    w2.add(new Wheel2(wheel.sprite.getX() + wheel.sprite.getWidth() / 2, wheel.sprite.getY() + wheel.sprite.getHeight() / 2));
                    w2.get(w2.size()-1).velocity.y=Math.abs(wheel.velocity.y);
                    w2.get(w2.size()-1).setFrame(wheel.getFrame());
                    w2.get(w2.size()-1).boundary.radius=10;
        ...
        if(!w4.isEmpty()) {
            for (ListIterator<Wheel4> iterator=w4.listIterator(); iterator.hasNext();) {
                ...
                if(Intersector.overlaps(wheel.boundary,fireboundary)) {
                    ...
                    w1.get(w1.size()-1).boundary.radius=20;
                    w1.add(new Wheel1(wheel.sprite.getX() + wheel.sprite.getWidth() / 2, wheel.sprite.getY() + wheel.sprite.getHeight() / 2));
                    w1.get(w1.size()-1).velocity.y=Math.abs(wheel.velocity.y);
                    w1.get(w1.size()-1).setFrame(wheel.getFrame());
                    w1.get(w1.size()-1).boundary.radius=20;
