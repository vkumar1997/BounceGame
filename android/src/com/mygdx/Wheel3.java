...
public class Wheel3 {
    ...
    public Wheel3(float posX, float posY) {
        sound = Gdx.audio.newSound(Gdx.files.internal("Swords_Collide-Sound_Explorer-2015600826.wav"));
        position = new Vector2(posX, posY);
        velocity = new Vector2((float) 0.12 * Gdx.graphics.getWidth()*(float)2.1, 0);
        acceleration = new Vector2(0, (float) -0.37 * Gdx.graphics.getHeight());
        sprite = new Sprite(new Texture(Gdx.files.internal("wheel3.PNG")));
        sprite.setPosition(posX, posY);
        boundary=new Circle(posX+25,posY+25,25);
    }
    ...
    public void spriteCollisionDetection(Sprite framesprite) {
        ...
        if (sprite.getY() < 27) {
            sound.play();
            velocity.y = (float) 0.4589 * Gdx.graphics.getHeight()/(float)1.4;
            position.y = 27;
            elapsedTimeY = 0;
        }
